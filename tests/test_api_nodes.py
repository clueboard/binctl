...
class TestNodeCreate:
    def test_create_minimal(self, client):
        resp = client.post('/v1/nodes', json={'label': 'shelf-A'})
    ...
    def test_create_missing_label(self, client):
        resp = client.post('/v1/nodes', json={'description': 'oops'})
        assert resp.status_code == 400

    def test_create_empty_label(self, client):
        resp = client.post('/v1/nodes', json={'label': ''})
        assert resp.status_code == 400

    def test_create_with_container_parent(self, client, make_node):
        container_id = make_node('room', is_container=True)
        resp = client.post('/v1/nodes', json={'label': 'shelf', 'parent_id': container_id})
        assert resp.status_code == 201
        assert resp.json()['parent_id'] == container_id

    def test_create_nonexistent_parent(self, client):
        resp = client.post('/v1/nodes', json={'label': 'shelf', 'parent_id': 99999})
        assert resp.status_code == 400

    def test_create_non_container_parent(self, client, make_node):
        item_id = make_node('item', is_container=False)
        resp = client.post('/v1/nodes', json={'label': 'child', 'parent_id': item_id})
        assert resp.status_code == 400

    def test_create_with_tags(self, client, make_node, make_tag):
        t1 = make_tag('electronics')
        t2 = make_tag('fragile')
        resp = client.post('/v1/nodes', json={'label': 'laptop', 'tag_ids': [t1, t2]})
        assert resp.status_code == 201
...
class TestNodeGet:
    def test_get_detail_full(self, client, make_node, make_tag):
        ...
        client.patch(f'/v1/nodes/{child_id}', json={'tag_ids': [tag_id]})
    ...
    def test_get_not_found(self, client):
        resp = client.get('/v1/nodes/99999')
...
class TestNodeList:
    ...
    def test_list_pagination(self, client, make_node):
        make_node('a')
        make_node('b')
        make_node('c')
        resp = client.get('/v1/nodes', params={'limit': 2, 'offset': 1})
        assert resp.status_code == 200
        body = resp.json()
        assert body['total'] == 3
    ...
    def test_list_invalid_limit(self, client):
        resp = client.get('/v1/nodes', params={'limit': 0})
        assert resp.status_code == 400


class TestNodePatch:
    def test_patch_label(self, client, make_node):
        node_id = make_node('old label')
        resp = client.patch(f'/v1/nodes/{node_id}', json={'label': 'new label'})
        assert resp.status_code == 200
        assert resp.json()['label'] == 'new label'

    def test_patch_not_found(self, client):
        resp = client.patch('/v1/nodes/99999', json={'label': 'x'})
        assert resp.status_code == 404

    def test_patch_set_parent(self, client, make_node):
        container_id = make_node('container', is_container=True)
        node_id = make_node('node')
        resp = client.patch(f'/v1/nodes/{node_id}', json={'parent_id': container_id})
        assert resp.status_code == 200
        assert resp.json()['parent_id'] == container_id

    def test_patch_clear_parent(self, client, make_node):
        container_id = make_node('container', is_container=True)
        node_id = make_node('node', parent_id=container_id)
        resp = client.patch(f'/v1/nodes/{node_id}', json={'parent_id': None})
        assert resp.status_code == 200
        assert resp.json()['parent_id'] is None

    def test_patch_self_parent(self, client, make_node):
        node_id = make_node('node', is_container=True)
        resp = client.patch(f'/v1/nodes/{node_id}', json={'parent_id': node_id})
        assert resp.status_code == 400

    def test_patch_cycle(self, client, make_node):
        ...
        resp = client.patch(f'/v1/nodes/{a_id}', json={'parent_id': b_id})
        assert resp.status_code == 400

    def test_patch_indirect_cycle(self, client, make_node):
        # A → B → C; try to set C as parent of A
        a_id = make_node('A', is_container=True)
        b_id = make_node('B', is_container=True, parent_id=a_id)
        c_id = make_node('C', is_container=True, parent_id=b_id)
        resp = client.patch(f'/v1/nodes/{a_id}', json={'parent_id': c_id})
        assert resp.status_code == 400

    def test_patch_replace_tags(self, client, make_node, make_tag):
        t1 = make_tag('tag1')
        t2 = make_tag('tag2')
        t3 = make_tag('tag3')
        node_id = make_node('node', tag_ids=[t1, t2])
        resp = client.patch(f'/v1/nodes/{node_id}', json={'tag_ids': [t2, t3]})
        assert resp.status_code == 200
        tag_names = {t['name'] for t in resp.json()['tags']}
        assert tag_names == {'tag2', 'tag3'}

    def test_patch_clear_tags(self, client, make_node, make_tag):
